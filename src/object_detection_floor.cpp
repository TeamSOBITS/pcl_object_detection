...
ObjectDetectionFloorNode::ObjectDetectionFloorNode(const rclcpp::NodeOptions& options) : BaseNode<sensor_msgs::msg::PointCloud2>("object_detection_floor", options){
...
void ObjectDetectionFloorNode::activate() {
    
    YAML::Node config = YAML::LoadFile(floor_param_path_);
    
    pcp_.reset();
    pcp_ = std::make_shared<pcl_object_detection::PointCloudProcessor>("point_cloud_processor_2");
    ...
    RCLCPP_INFO(this->get_logger(), "ObjectDetectionFloorNode activated");    
}
...
void ObjectDetectionFloorNode::deactivate() {
    ...
    // RCLCPP_INFO(this->get_logger(), "ObjectDetectionFloorNode deactivated");
}
