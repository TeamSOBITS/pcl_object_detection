...
TableDetectionComponent::CallbackReturn TableDetectionComponent::on_activate(const rclcpp_lifecycle::State &) {
  ...
  // Explicit callback group so the subscription is collected into the executor's
  // wait set even though it is created during a lifecycle transition while the
  // component_container_mt executor is already spinning this node.
  if (!cb_group_) {
