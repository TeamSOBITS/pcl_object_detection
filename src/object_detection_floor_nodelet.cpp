...
namespace pcl_object_detection {
    class ObjectDetectionFloor : public nodelet::Nodelet {
        private:
            ros::NodeHandle nh_;
            ros::NodeHandle pnh_;
...
void pcl_object_detection::ObjectDetectionFloor::callbackDynamicReconfigure(pcl_object_detection::PCLParameterConfig& config, uint32_t level) {
...
bool pcl_object_detection::ObjectDetectionFloor::callbackSubscriberSwitch( pcl_object_detection::RunCtrl::Request &req, pcl_object_detection::RunCtrl::Response &res ) {
    if ( req.request ) {
...
void pcl_object_detection::ObjectDetectionFloor::callbackCloud(const sensor_msgs::PointCloud2ConstPtr& cloud_msg) {
    ...
    NODELET_INFO("\033[1;32m[ ObjectDetectionFloor ]\tObject count = %d \033[m", object_num );
    return;
}

void pcl_object_detection::ObjectDetectionFloor::onInit() {
    nh_ = getNodeHandle();
    pnh_ = getPrivateNodeHandle();

    pcp_.reset( new pcl_object_detection::PointCloudProcessor );

    srv_subscriber_switch_ = nh_.advertiseService( "object_detection_floor_run_ctr", &ObjectDetectionFloor::callbackSubscriberSwitch, this);

    // dynamic_reconfigure :
    server_ = new dynamic_reconfigure::Server<pcl_object_detection::PCLParameterConfig>(pnh_);
    f_ = boost::bind(&ObjectDetectionFloor::callbackDynamicReconfigure, this, _1, _2);
...
PLUGINLIB_EXPORT_CLASS(pcl_object_detection::ObjectDetectionFloor, nodelet::Nodelet)
