...
/**
 * @class ShelfDetectionComponent
 * @brief Managed ROS 2 Lifecycle Component for detecting objects within a shelf.
 * 
 * Extracts a specific 3D volume, removes the horizontal shelf board, and optionally
 * removes the vertical back wall before clustering the remaining objects.
 */
class ShelfDetectionComponent : public rclcpp_lifecycle::LifecycleNode {
public:
  using PointT = pcl::PointXYZ;
  using PointCloud = pcl::PointCloud<PointT>;

  explicit ShelfDetectionComponent(const rclcpp::NodeOptions & options);
...
private:
  ...
  PointCloud::Ptr cloud_shelf_zone_;
  ...
  struct {
    ...
    // Shelf physical bounds
    double shelf_x_min, shelf_x_max;
    double shelf_y_min, shelf_y_max;
    double shelf_z_min, shelf_z_max;

    // Plane segmentation parameters
    double plane_dist_threshold;
    bool remove_back_wall;
